...
auxtext='all_categories_2mm_FS'; % Put whatever you might want as a prefix for the generated nifti files
mms='2mm';
subs=11;
blocks=6;
cats=4;
...
perm=0;
group=[1 4 9 10 11];
resultType='intersubject'; % intrasubject or intersubject
...
maskfilename= '/m/nbe/scratch/braindata/jaalho/psykoosi/visuaaliset_mielikuvat/mask_verrokit.nii';
template=load_nii('./importance_visualization/aux_files/MNI152_T1_2mm.nii'); % The template to copy to the header

%%
mask=load_nii(maskfilename);
inmask=find(mask.img);
dirtolook=sprintf('%sS_%s_%d_sub1_lr0.00/',resultPath,mms,perm);
res=dir([dirtolook '/rep*']);
load([dirtolook res(1).name '/' 'impos.mat'])
group_data=zeros(length(inmask),size(impos,3),subs);
for subi=1:subs
    data=zeros(length(inmask),size(impos,3),size(impos,1),reps,blocks);
    for blocki=1:blocks
        dirtolook=sprintf('%sS_%s_%d_sub%d_lr0.00/',resultPath,mms,perm,subi);
        res=dir([dirtolook '/rep*']);
        for resi=1:reps % This defines the number of repetitions to read, if they haven't run you'll get an error
            ...
            data(voxels_to_keep,:,:,resi,blocki)=permute(impos,[2 3 1]); % data(voxels,categories,samples,repetitions,blocks)           
        end
    end
...
for subi=1:subs
    ...
    for cati=1:size(group_data,2)
        ...
        filename=[curpath '/importance_visualization/niis/' auxtext '_' resultType '_sub' num2str(subi) '_cat' num2str(cati) '.nii']; % Set the filename for the low resolution nifti
...
for cati=1:size(data,2)    
    ...
    filename=[curpath '/importance_visualization/niis/' auxtext '_' resultType '_cat' num2str(cati) '_' id '.nii']; % Set the filename for the low resolution nifti
